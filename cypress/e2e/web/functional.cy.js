...
describe("Should test at functional level", () =>{
    ...
    it('Should update an account', function ()  {
        cy.accessAccountMenu()
        cy.xpath(locators.ACCOUNTS.FN_XP_BTN_UPDATE(Cypress.env('account_to_be_updated_name'))).click()
    ...
    it('Should delete an account', function () {
        cy.accessAccountMenu()
        cy.xpath(locators.ACCOUNTS.FN_XP_BTN_DELETE(Cypress.env('account_name_updated'))).click()
        cy.fixture('toast_success_messages').as('toast').then(() => {
    ...
    it('Should create a transaction', function() {
        ...
        cy.get(locators.ACCOUNT_TRANSACTION.ACCOUNT).select(Cypress.env('account_name_to_make_transaction'))
        ...
        cy.fixture('toast_success_messages').as('toast').then(() => {
            cy.get(locators.TOAST_MESSAGE).should('contain', this.toast.transaction)
        })

        // VERIFYING IF THE NEW TRANSACTION WAS RECORDED
        cy.get(locators.EXTRACT.LINES).should('have.length', 7)
        cy.xpath(locators.EXTRACT.FN_XP_SEARCH_TRANSACTION('Desc', '123,00')).should('exist')
    })

    it('Should get balance', function() {
        cy.get(locators.MENU.HOME).click()
        cy.xpath(locators.BALANCE.FN_XP_ACCOUNT_BALANCE(Cypress.env('account_name_to_get_balance'), '534,00')).should('exist')
    })

    it('Should delete a transaction', function() {
        cy.get(locators.MENU.EXTRACT).click()
        cy.xpath(locators.EXTRACT.FN_XP_DELETE_TRANSACTION(Cypress.env('transaction_name_desc_to_be_deleted'))).click()
